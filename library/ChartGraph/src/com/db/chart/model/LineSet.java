...
public class LineSet extends ChartSet{
	...
	public LineSet(@NonNull String[] labels, @NonNull float[] values, @NonNull float[] customLabel, @NonNull float[] customValues){
		super();
		init();
		...
		if(customLabel == null || customValues == null)
			throw new IllegalArgumentException("Custom Labels or/and custom values can't be null.");
		if(customLabel.length != customValues.length)
			throw new IllegalArgumentException("Custom arrays size doesn't match.");

		for(int i = 0; i < customLabel.length; i++){
			addMiddlePoint(customLabel[i], customValues[i]);
		}
	}
	...
	public void addMiddlePoint(float label, float value){
		this.addMiddlePoint(new Point(String.valueOf(label), value));
	}
	...
	public void addMiddlePoint(@NonNull Point point){
		this.addMiddleEntry(point);
	}
	/**
	 * If line dashed.
	 *
	...
	public LineSet setDotsColor(@ColorInt int color){
		...
		for(ChartEntry e : getMiddleEntries())
			e.setColor(color);
        return this;
	}
