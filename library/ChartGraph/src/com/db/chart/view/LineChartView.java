...
public class LineChartView extends ChartView {
	...
	private void drawPoints(Canvas canvas, LineSet set) {
		...
		int end = set.getEnd();
		if( set.size() > 0) {
			Point start_dot = (Point) set.getEntry(begin);
			Point end_dot = (Point) set.getEntry(end - 1);
			for (int i = 0; i < set.size_Middle(); i++) {

				dot = (Point) set.getMiddleEntry(i);

				if (dot.isVisible() && dot.getX() >= start_dot.getX() && dot.getX() <= end_dot.getX()) {
			...
			for (int i = begin; i < end; i++) {
				...
				dot = (Point) set.getEntry(i);// dot = (Point) set.getEntry(i);
	...
	private void drawLine(Canvas canvas, LineSet set) {
		...
		if( set.size()>0) {
			...
			Point end_dot = (Point) set.getEntry(end - 1);
			Point dot;
			for (int i = 0; i < set.size_All(); i++) {

				dot = (Point) set.getEntryInAll(i);
